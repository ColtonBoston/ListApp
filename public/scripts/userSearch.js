          ...
          if (users.length > 0){
            ...
            users.forEach(function(user){
              ...
              if (currentUser && currentUser.friends.indexOf(user._id) > -1){
                usersList.append("<li class='list-group-item li-user'><span class='li-user-username'><a href='/users/profile/" + user._id + "'>" + user.rawUsername + "</a></span><button class='btn-friend btn btn-danger btn-sm pull-right' type='submit' data-user-id='" + user._id + "'>Remove Friend</button></li>");
                ...
                usersList.append("<li class='list-group-item li-user'><span class='li-user-username'><a href='/users/profile/" + user._id + "'>" + user.rawUsername + "</a></span><button class='btn-friend btn btn-primary btn-sm pull-right type='submit' data-user-id='" + user._id + "'>Add Friend</button></li>");
                searchResults.removeClass("hide-search-results");
              }
            });
          } else {
